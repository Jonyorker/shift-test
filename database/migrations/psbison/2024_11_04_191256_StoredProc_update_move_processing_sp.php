...
return new class extends Migration
{
    ...
    {
        // Add in Custom data types first

        $sql = 'CREATE TYPE UpdateMoveProcessingLegs AS TABLE(
        ...
        $sql = 'CREATE TYPE UpdateMoveProcessingStops AS TABLE(
        ...
        $sql = 'CREATE TYPE TMWTable_char6 AS TABLE(
            [KeyField] [char](6) NULL
            )';

        DB::connection('psbison')->getPdo()->exec($sql);

        $sql = "CREATE PROCEDURE update_move_processing_sp(@mov INTEGER, @assetassignmnet CHAR(1))
        AS
        ...
        ********************************************************************************************************************/
             ...
             SELECT @HourlyPaywithBonus = COALESCE(@HourlyPaywithBonus, '0'),
              ...
              -- bison td/kl dont disturb the dead...
                ...
                select @maxdt=max(stp_arrivaldate) from stops (nolock) where mov_number=@mov
            ...
            (
                ...
                --THEN 'STD'                                                                                            -- Status = STP not CMP and at least first stop arrived
                  WHEN FS.stp_departure_status = 'DNE' THEN 'STD'
                  WHEN FS.stp_status= 'DNE' then 'DSP'
                --end bison change kl	-- Status = STP not CMP and at least first stop arrived
                                    WHEN @inbond = 1
    ...
    {
        $sql = 'DROP PROCEDURE IF EXISTS update_move_processing_sp';
        DB::connection('psbison')->getPdo()->exec($sql);

        $sql = 'DROP TYPE IF EXISTS UpdateMoveProcessingStops';
        DB::connection('psbison')->getPdo()->exec($sql);

        $sql = 'DROP TYPE IF EXISTS UpdateMoveProcessingLegs';
        DB::connection('psbison')->getPdo()->exec($sql);

        $sql = 'DROP TYPE IF EXISTS TMWTable_char6';
        DB::connection('psbison')->getPdo()->exec($sql);
