...
return new class extends Migration
{
    ...
    {
        $sql = "CREATE proc update_assetassignment @mov_number int
        as
        /*
        ...
        */
        ...
        DECLARE @assethold TABLE
               (rec_id				int	identity,
                ...
                fgt_number			INTEGER NULL)  --TTESUITE-60122 MBR 07/12/21
        ...
        Declare @donotpay_status as varchar(6),
            ...
            @lgh_type3 varchar(6),
            @minlgh int,
            @asset_as_freight char (1)	--TTESUITE-60122 MBR 07/12/21
        ...
        -- bison td/kl dont disturb the dead...
        declare @maxdt datetime
        select @maxdt=max(stp_arrivaldate) from stops with(nolock) where mov_number=@mov_number
        if @maxdt>'2020-06-01' select @completeondeparture='Y'
        --end bison changes

        --TTESUITE-60122 MBR 07/12/21
        SELECT @asset_as_freight = UPPER(LEFT(ISNULL(gi_string1, 'No'), 1))
          FROM generalinfo
         WHERE gi_name = 'AssetsAsFreight'
        SELECT @asset_as_freight = ISNull (@asset_as_freight, 'N')
        ...
        /* TTESUITE-60122 MBR 07/12/21
           Add Assets */
        if @asset_as_freight = 'Y'
        BEGIN
            insert @assethold (lgh_number, asgn_number, asgn_type, asgn_id, asgn_date,
                asgn_eventnumber, asgn_controlling, asgn_status, asgn_dispdate,
                asgn_enddate, asgn_dispmethod, s.mov_number, pyd_status, actg_type,
                evt_number, last_evt_number, first_seq, last_seq,
                completed_seq, started_seq, loaded, fgt_number)
            SELECT s.lgh_number, 0, 'AST', fd.fgt_asset, e.evt_startdate,
                null, 'N', '', l.lgh_dispatchdate,
                l.lgh_enddate, null, s.mov_number, 'DNP', 'N',
                evt_number,
                (SELECT top 1 evt.evt_number
                    FROM event evt join stops stp on stp.stp_number = evt.stp_number
                    WHERE s.mov_number = stp.mov_number
                      AND (evt.evt_pu_dr = 'DRP' OR stp.stp_event = 'XDU')
                      ORDER BY stp.stp_mfh_sequence desc) last_evt_number,
                evt_sequence,
                (SELECT top 1 evt.evt_sequence
                    FROM event evt join stops stp on stp.stp_number = evt.stp_number
                    WHERE s.mov_number = stp.mov_number
                      AND (evt.evt_pu_dr = 'DRP' OR stp.stp_event = 'XDU')
                      ORDER BY stp.stp_mfh_sequence desc) last_seq,
                null, null, 'Y', fd.fgt_number
            FROM freightdetail fd
                join stops s on fd.stp_number = s.stp_number
                join event e on s.stp_number = e.stp_number
                join legheader l on l.lgh_number = s.lgh_number
            where fd.fgt_asset is not null
        ...
        BEGIN
           ...
           /* TTESUITE-60122 MBR 07/12/21: Update Assets */
           update @assethold
                set asgn_status =
                        CASE WHEN lg.lgh_outstatus = 'AVL' THEN 'PLN'
                        ELSE lg.lgh_outstatus END,
                    last_dne_evt_number =
                        case when completed_seq < first_seq then 0
                        when completed_seq < last_seq then /*get event from completed_seq */
                            (select min(evt_number) from @trlevent t where t.seq = completed_seq)
                ...
                from legheader lg join @assethold ah on ah.lgh_number = lg.lgh_number
                where asgn_type = 'AST'
        END
        ELSE
        BEGIN
        ...
        begin
            ...
            INSERT INTO assetassignment(lgh_number, asgn_type, asgn_id,
                ...
                asgn_trl_last_asgn, last_evt_number, last_dne_evt_number, next_opn_evt_number, asgn_branch, fgt_number)
    ...
    {
        $sql = 'DROP PROCEDURE IF EXISTS update_assetassignment';
        DB::connection('psbison')->getPdo()->exec($sql);
    }
};
